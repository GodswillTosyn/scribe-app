  ...
  if (!open) {
    return (
      <div className="shrink-0 flex flex-col items-center gap-3 w-10 border-l h-full py-3" style={{ borderColor: "var(--border)", background: "var(--surface)" }}>
        ...
        <button onClick={() => { onToggle(); setTab("ai"); }} title="Research AI" className="w-8 h-8 rounded-lg flex items-center justify-center transition-colors"
          onMouseEnter={(e) => (e.currentTarget.style.background = "var(--hover)")} onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="url(#ai-g2)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
